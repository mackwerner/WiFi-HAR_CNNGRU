...
# === Preprocessing (Fix CSI tensors) ===

def preprocess_csi(arr: np.ndarray) -> np.ndarray:
    arr = np.asarray(arr)

    if arr.ndim == 3:
        # Make time the first dimension, then flatten the rest.
        arr = np.moveaxis(arr, -1, 0)  # time now axis 0: (T, ..., ...)
        T = arr.shape[0]
        F = int(np.prod(arr.shape[1:]))
        arr = arr.reshape(T, F)

    elif arr.ndim == 2:
        # Choose the longer dimension as time so final shape is (T, F)
        if arr.shape[0] < arr.shape[1]:
            arr = arr.T  # make rows = time
...
def load_split(split):
    ...
    for label, cls_name in enumerate(CLASS_NAMES):
        cls_dir = os.path.join(split_dir, cls_name.lower())
        ...
        for fname in sorted(os.listdir(cls_dir)):
            ...
            data_key = data_keys[0]  # safe unless empty
            raw = mat[data_key]
            arr = preprocess_csi(raw)
...
# === Load NTU-Fi HAR data from folder ===

X_train, y_train = load_split("train")
X_val,   y_val   = load_split("test")

# --- Downsample in time: keep every 4th frame (2000 -> 500) ---
X_train = X_train[:, ::4, :]   # (936, 500, 342)
X_val   = X_val[:,   ::4, :]   # (264, 500, 342)
